...
diss_final_concs <- read_csv("intermediates/NFEX_diss_full_final_concs_after_ee.csv") %>%
  ...
  summarise(mean_nM = mean(final_nM)) %>%
  ungroup() %>%
...
diss_final_concs$day <- factor(diss_final_concs$day)

# Identifier for the compounds
diss_final_concs$compound_id <- as.integer(as.factor(diss_final_concs$compound_name))
...
#   geom_col(position = "fill", color = "black")
#
# Glutamine_NO3_final_concs_plot <- final_concs %>%
...
NO3_compound_group_plot <- diss_final_concs %>%
  ...
  labs(title = "Dissolved Metabolite Mass Isotope Distribution treated by NO3",
       x = "Hour",
...
NH4_compound_group_plot <- diss_final_concs %>%
  ...
  geom_col(position = "fill", color = "black") +
  facet_wrap(~ compound_group, scales = "free_y", nrow = 1) +
  labs(title = "Dissolved Metabolite Mass Isotope Distribution treated by NH4",
       x = "Hour",
       y = "Mean Concentration (nM)",
       fill = "Isotope Label") +
  theme(legend.key.size = unit(0.5, "cm")) +
...
title = "Dissolved Metabolite Mean Concentration by Treatment and Hour"
