...
parti_final_concs <- read_csv("intermediates/NFEX_parti2_final_concs.csv") %>%
...
part_final_concs <- parti_final_concs %>%
  mutate(compound_group = case_when(
    grepl("Carnitine", compound_name) ~ "Carnitine",
    grepl("Choline", compound_name) ~ "Choline",
    grepl("Dimethylglycine", compound_name) ~ "Dimethylglycine",
    grepl("Ectoine", compound_name) ~ "Ectoine",
    grepl("Glycine", compound_name) ~ "Glycine",
    grepl("Lysine", compound_name) ~ "Lysine",
    grepl("Proline", compound_name) ~ "Proline",
    grepl("Glycine", compound_name) ~ "Glycine",
    grepl("Proline Betaine", compound_name) ~ "Proline Betaine",
    grepl("Trimethylamine N-Oxide", compound_name) ~ "TMAO",
    TRUE ~ "Other"
  ))
...
# Filter and plot for all compounds grouped by compound_group
Urea_compound_group_plot <- part_final_concs %>%
  mutate(iso_label=paste0(N, "x15N, ", C, "x13C")) %>%
...
# Filter and plot for all compounds grouped by compound_group
NH4_compound_group_plot <- part_final_concs %>%
  mutate(iso_label=paste0(N, "x15N, ", C, "x13C")) %>%
